...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, MFMailComposeViewControllerDelegate {
    ...
    let rezeptbuch = Rezeptbuch()           // Zugriff auf die Container-Klasse
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        refreshControl.attributedTitle = NSAttributedString(string: "Neue Rezepte laden...")
        refreshControl.addTarget(self, action: #selector(ViewController.refreshRezepte(_:)), forControlEvents: .ValueChanged)
    ...
    //
    // editactions
    // Öffnen mit Swipe-Geste!
    // FEHLER bei Aktion im Simulator: MailCompositionService wurde unerwartet beendet
